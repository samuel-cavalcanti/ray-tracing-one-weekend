use ray_tracing_one_weekend::{
    random,
    vec3::{self,  random_unit_hemisphere},
    write_color_wih_gamma_correction, Camera, Color, Float, HitRecord, Hittable, Point3, Ray,
    Sphere,
};
...
fn ray_color<H: Hittable>(ray: &Ray, world: &Vec<H>, depth: i32) -> Color {
    ...
    if let Some(rec) = hit_anything(ray, world) {
        let target =  random_unit_hemisphere(&rec.normal);
...
fn main() {
    ...
    imgbuf
        .save("images/diffuse_sphere_with_hemispherical_scattering.png")
        .unwrap();
}
